...
<svelte:head>
	<title>Protest Listing - Upcoming Protests & Demonstrations</title>
</svelte:head>

<!-- Hero Section -->
<section class="relative overflow-hidden bg-[#EEEEEE] dark:bg-gray-900 py-12 px-4 md:py-16 transition-colors border-b-4 border-black dark:border-white">
	...
	<div class="max-w-7xl mx-auto text-center relative">
		<h1 class="text-3xl md:text-4xl lg:text-5xl font-bold text-black dark:text-white mb-4 tracking-tight uppercase">
			Upcoming Protests & Demonstrations
		</h1>
		...
		<form on:submit={handleSearch} class="max-w-2xl mx-auto">
			...
			<div class="relative flex items-center bg-white rounded-full shadow-md hover:shadow-lg transition-shadow">
				...
				<button
					...
					class="lg:hidden absolute right-14 w-10 h-10 rounded-full text-black/60 dark:text-white/60 hover:text-black dark:hover:text-white hover:bg-gray-50 dark:hover:bg-gray-600 flex items-center justify-center transition-colors"
				...
				<button
					type="submit"
					class="absolute right-2 w-10 h-10 md:w-12 md:h-12 rounded-full bg-[#E10600] hover:bg-[#C10500] text-white flex items-center justify-center hover:scale-105 active:scale-95 transition-transform shadow-md"
					aria-label="Search"
				>
		...
		<main class="space-y-6">
			...
			<div class="lg:hidden flex items-center gap-3">
				<button
					type="button"
					on:click={toggleMobileFilters}
					class="flex items-center gap-2 px-4 py-3 rounded-xl bg-white dark:bg-gray-800 border-2 border-black dark:border-white text-black dark:text-white hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black font-medium shadow-sm hover:shadow-md transition-all"
				>
					<Icon icon="heroicons:funnel" class="w-5 h-5" />
					Filters
				</button>
				<div class="flex-1 text-sm text-black/60 dark:text-white/60">
					{#if data.total > 0}
						<span class="font-medium text-black dark:text-white">{data.total}</span> events
					{:else}
						No events
					{/if}
				</div>
			...
			<div class="hidden lg:flex items-center justify-between">
				<p class="text-sm text-black/60 dark:text-white/60">
					{#if data.total > 0}
						Found <span class="font-medium text-black dark:text-white">{data.total}</span> upcoming events
					{:else}
						No events found
					{/if}
				</p>
			...
			{#key key}
				...
				{:else if data.error}
					...
					<div class="text-center py-16">
						...
						<h3 class="text-xl font-medium text-black dark:text-white mb-2">Error Loading Events</h3>
						<p class="text-black/60 dark:text-white/60 mb-6">
							{data.error}
						</p>
						<button
							on:click={() => window.location.reload()}
							class="px-6 py-3 rounded-xl bg-[#E10600] hover:bg-[#C10500] text-white font-medium hover:shadow-lg transition-all"
						>
							Try Again
						</button>
					</div>
				{:else}
					<!-- Empty State -->
					<div class="text-center py-16">
						<div class="inline-block p-4 rounded-full bg-[#EEEEEE] dark:bg-gray-800 border-2 border-black/10 dark:border-white/10 mb-4">
							<Icon icon="heroicons:calendar-days" class="w-12 h-12 text-black dark:text-white" />
						</div>
						<h3 class="text-xl font-medium text-black dark:text-white mb-2">No Events Found</h3>
						<p class="text-black/60 dark:text-white/60 mb-6">
			...
			{#if data.total > data.limit}
				<div class="flex flex-col items-center gap-4 pt-8">
					<!-- Results Info -->
					<p class="text-sm text-black/60 dark:text-white/60">
						Showing {((data.page - 1) * data.limit) + 1}–{Math.min(data.page * data.limit, data.total)} of {data.total} events
					</p>
					...
					<nav class="flex w-full items-center gap-1 sm:gap-1" aria-label="Pagination">
						...
						<button
							...
							class="px-2 sm:px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 text-black dark:text-white hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-1"
						...
						{#if data.page > 3 && totalPages > 5}
							<button
								on:click={() => goToPage(1)}
								class="w-8 h-8 sm:w-10 sm:h-10 rounded-lg border border-gray-200 dark:border-gray-700 text-black dark:text-white hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors flex items-center justify-center text-sm sm:text-base"
							...
							<button
								...
								class="w-8 h-8 sm:w-10 sm:h-10 rounded-lg transition-colors flex items-center justify-center font-medium text-sm sm:text-base
									{pageNum === data.page 
										...
										: 'border border-gray-200 dark:border-gray-700 text-black dark:text-white hover:bg-gray-50 dark:hover:bg-gray-700'}"
					...
					<div class="flex items-center gap-2 text-xs sm:text-sm sm:hidden">
						<label for="page-input" class="text-black/60 dark:text-white/60">Go to page:</label>
						<input
							id="page-input"
							type="number"
							min="1"
							...
							class="w-12 px-1 py-1 text-center border border-gray-200 dark:border-gray-700 rounded bg-white dark:bg-gray-800 text-black dark:text-white text-xs"
						/>
						<span class="text-black/60 dark:text-white/60">of {totalPages}</span>
					</div>
				</div>
			{/if}
		</main>
...
<a
	href="/events/create"
	class="fixed bottom-6 right-6 w-14 h-14 rounded-full bg-[#E10600] hover:bg-[#C10500] text-white text-2xl shadow-lg hover:shadow-xl transform hover:scale-110 active:scale-95 transition-all duration-200 flex items-center justify-center lg:hidden z-50"
