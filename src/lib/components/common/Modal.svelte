<script>
  import { createEventDispatcher } from 'svelte';
  import { fade, scale } from 'svelte/transition';
  import Icon from '$lib/components/common/Icon.svelte';

  export let isOpen = false;
  export let title = '';
  export let maxWidth = 'md'; // sm, md, lg, xl

  const dispatch = createEventDispatcher();

  const maxWidths = {
    sm: 'max-w-sm',
    md: 'max-w-md',
    lg: 'max-w-lg',
    xl: 'max-w-xl'
  };

  function close() {
    isOpen = false;
    dispatch('close');
  }

  function handleBackdropClick(e) {
    if (e.target === e.currentTarget) {
      close();
    }
  }

  function handleKeydown(e) {
    if (e.key === 'Escape' && isOpen) {
      close();
    }
  }

  function handleBackdropKeydown(e) {
    if (e.target === e.currentTarget && (e.key === 'Enter' || e.key === ' ')) {
      e.preventDefault();
      close();
    }
  }
</script>

<svelte:window on:keydown={handleKeydown} />

{#if isOpen}
  <!-- Backdrop -->
  <div
    class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4"
    on:click={handleBackdropClick}
    on:keydown={handleBackdropKeydown}
    tabindex="-1"
    role="presentation"
    transition:fade={{ duration: 200 }}
  >
    <!-- Modal -->
    <div
      class="w-full {maxWidths[maxWidth]} bg-white dark:bg-stone-800 rounded-2xl shadow-2xl overflow-hidden"
      role="dialog"
      aria-modal="true"
      transition:scale={{ duration: 200, start: 0.95 }}
    >
      <!-- Header -->
      {#if title}
        <div class="flex items-center justify-between p-6 border-b border-stone-100 dark:border-stone-700">
          <h2 class="text-xl font-medium text-black dark:text-white">
            {title}
          </h2>
          <button
            on:click={close}
            class="w-8 h-8 rounded-full hover:bg-stone-100 dark:hover:bg-stone-700 flex items-center justify-center transition-colors"
            aria-label="Close modal"
          >
            <Icon icon="heroicons:x-mark" class="w-5 h-5 text-black/60 dark:text-white/60" />
          </button>
        </div>
      {/if}

      <!-- Content -->
      <div class="p-6">
        <slot />
      </div>
    </div>
  </div>
{/if}
